...
import me.bakuleva.coursework3.model.TypeOfTransaction;
...
public class SocksServiceImpl implements SocksService {
    ...
    public boolean addSocks(Sock[] newSocks) {
        if (newSocks != null) {
            for (int i = 0; i < newSocks.length; i++) {
                ...
                transactionService.addTransaction(new Transactions(TypeOfTransaction.ADD, newSocks[i].getQuantity(),
                        newSocks[i].getSize(), newSocks[i].getCottonPercentage(), newSocks[i].getColor()));
            }
            saveToFile();
            return true;
        }
        ...
        if (takenSocks != null) {
            ...
            for (int i = 0; i < takenSocks.length; i++) {
                ...
                if (socks.containsValue(takenSocks[i])
                    ...
                    if (transactionService.addTransaction(new Transactions(TypeOfTransaction.PICK_UP, takenSocks[i].getQuantity(),
                            takenSocks[i].getSize(), takenSocks[i].getCottonPercentage(), takenSocks[i].getColor()))) {
                    ...
                    answer.append("На складе недостаточно носков: ")
                            ...
                            .append(takenSocks[i].getCottonPercentage()).append("%; ")
    ...
    public Collection<Sock> getSocks(String color, float size, int cottonMin, int cottonMax) {
        ...
        for (Sock sock :
                socks.values()) {
            if (sock.getColor() == Color.valueOf(color.toUpperCase()) &&
        ...
        for (Sock deletedSock :
            ...
            if (socks.containsValue(deletedSock)) {
                ...
                if (newQuantity >= 0) {
                    ...
                    //добавляем транзакцию
                    if (transactionService.addTransaction(new Transactions(TypeOfTransaction.DELETE, deletedSock.getQuantity(),
                            deletedSock.getSize(), deletedSock.getCottonPercentage(), deletedSock.getColor()))) {
                        System.out.println("Транзакция отправлена на добавление!");
                    } else {
                        System.out.println("Транзакция не прошла на добавление!");
                    }
        ...
        try (Writer writer = Files.newBufferedWriter(path, StandardOpenOption.APPEND)) {
            ...
            for (Sock sock :
                ...
                writer.append("Цвет носков: ").append(sock.getColor().getText()).append("\n")
                        ...
                        .append("Процент содержания хлопка: ").append(String.valueOf(sock.getCottonPercentage())).append("\n")
